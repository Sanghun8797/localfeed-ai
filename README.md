...
LocalFeed AI는 당근마켓과 같은 지역 기반 서비스의 홈 피드를 가정하여 만든 개인화 추천 시스템입니다. 사용자 행동 로그, 게시글 정보, 사용자 선호 카테고리, 선호 동네, 가격대 정보를 활용하여 사용자별로 다른 추천 피드를 제공합니다. 이 프로젝트는 단순 인기글 추천에서 시작해 콘텐츠 기반 추천, 사용자 행동 기반 개인화 추천, 하이브리드 추천 모델까지 단계적으로 구현했습니다.
