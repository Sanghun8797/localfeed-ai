...
* 사용자 입력 조건 기반 개인화 추천 기능 구현
* 카테고리와 가격 범위를 먼저 필터링한 뒤 추천 점수 계산
* 조회수, 관심 수, 채팅 수를 활용한 인기도 반영
* 등록일 기반 최신성 반영
* 추천 점수를 100점 만점과 적합도 라벨로 변환하여 사용자 이해도 개선
* 추천 카드 클릭 시 상세페이지로 이동하는 서비스 흐름 구현
* CSV 기반 데이터 생성 후 PostgreSQL 테이블 적재 및 SQL 조회 API 구현
* FastAPI와 정적 웹 UI를 Render에 배포
